...
|------|-------|--------|------|
| `MAX_DENS_GLOBAL` (REF) | **193.0** | `config.py` | Fixed composite reference in $\log_{10}(1+D_{\mathrm{blend}}\sqrt{M}/\mathrm{REF})$. Recalibrated in Task 8c from 575.0 (pitch-gated era) so all-pitched totals keep the prior order of magnitude — **not** Qty or table size. |
...
| `DEFAULT_WEIGHT_FACTOR` | 0.5 | `config.py` | Instrument vs interval blend inside $D_{\mathrm{blend}}$ |

**Unified composite (Task 8c):** one formula for pitched, unpitched, and mixed slices — $D_{\mathrm{blend}}=\texttt{density.weighted}$, then mass boost / REF. No unpitched-only fallback. Traceability: `CHANGES.md`.
