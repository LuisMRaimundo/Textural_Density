...
| Committed dynamic ladders (no runtime GPR) | **Changed (2026-08-03; data-faithful rebuild 2026-08-08/09)** — table lookup only; pitched table-backed modules commit full 10-level Dynamics_predicter ladders (measured anchors verbatim, not forced monotone); unpitched percussion uses pitch-independent `DYNAMIC_CDM`; missing cells error; legacy GPR → `tools/legacy_gpr_dynamic_interpolation.py` (`CHANGES.md`) |
