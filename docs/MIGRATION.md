...
| Committed dynamic ladders (no runtime GPR) | **Changed (2026-08-03)** — table lookup only; full 10-level `Results` ladders for violin/viola/cello/dbass/flute/clarinet/bassoon/oboe; remaining sparse modules error on missing cells; legacy GPR → `tools/legacy_gpr_dynamic_interpolation.py` (`CHANGES.md`) |
