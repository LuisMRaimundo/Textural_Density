...
2. **Transferred (not measured) soft/loud anchors.** `violino_sul_ponticello` / `violino_art_harm` measure **mf only**; `pp`/`ff` are ratio transfers from violin arco (`mf_anchor_dynamic_extrapolation`). Registry `uncertainty="high"`; metadata warnings include the derived-anchor note and, for tail levels, `"tail computed from transferred anchors"`.
