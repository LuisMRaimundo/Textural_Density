...
- GPR-backed modules (`flute`, `oboe`, `clarinet`, `bassoon`, `trumpet`, `violin`, `viola`, `cello`, `double_bass`, …) use **externally sourced acoustic metadata** (sparse committed `pp`/`mf`/`ff` tables). Interior dynamics are GPR-interpolated at analysis time; tails use the saturating log-domain rule. This is **not** runtime audio analysis.
