...
- GPR-backed modules (`flute`, `oboe`, `violin`, `viola`, `cello`, `double_bass`, `clarinet`, …) use **externally sourced acoustic metadata** (sparse CDM or amplitude tables, GPR-interpolated). This is applied at score-analysis time — **not** runtime audio analysis.
