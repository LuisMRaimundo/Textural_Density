...
def test_full_ten_level_ladder_committed():
    ...
    assert next(reversed(mod.spectral_data)) == "G7"
...
def test_mf_lookup_returns_workbook_anchor():
    mod = importlib.import_module("instrumentos.violin_sul_ponticello")
    assert mod.calcular_densidade("G4", "mf") == pytest.approx(21.7286, rel=0, abs=1e-5)
    assert mod.calcular_densidade("A3", "mf") == pytest.approx(29.6028, rel=0, abs=1e-5)


def test_ff_lookup_returns_workbook_anchor():
    mod = importlib.import_module("instrumentos.violin_sul_ponticello")
    assert mod.calcular_densidade("G4", "ff") == pytest.approx(24.0575, rel=0, abs=1e-5)


def test_pp_is_measured_workbook_anchor():
    mod = importlib.import_module("instrumentos.violin_sul_ponticello")
    assert mod.calcular_densidade("G4", "pp") == pytest.approx(20.8974, rel=0, abs=1e-5)
...
def test_pipeline_accepts_violin_sul_ponticello():
    ...
    assert densities[0] == pytest.approx(21.7286, rel=0, abs=1e-5)
